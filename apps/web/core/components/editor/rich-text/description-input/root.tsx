...
type Props = {
  ...
  /**
   * @description Issue name/title, used to auto-generate a description via AI
   */
  issueName?: string;
};
...
export const DescriptionInput = observer(function DescriptionInput(props: Props) {
  ...
  return (
    <>
      ...
      {!disabled && config?.has_llm_configured && projectId && (
        <div className="flex items-center justify-end gap-2 pt-2">
          {issueName && issueName.trim() !== "" && (
            <button
              type="button"
              className={cn(
                "flex items-center gap-1 rounded-sm px-2 py-1 text-caption-sm-regular text-white transition-opacity hover:opacity-90",
            ...
            >
              {iAmFeelingLucky ? (
                ...
                <>
                  <Sparkle className="h-3 w-3" /> Generate Description
                </>
              )}
            </button>
          )}
          ...
          <GptAssistantPopover
            ...
            handleClose={() => setGptAssistantModal((prev) => !prev)}
            onResponse={(response) => handleAiAssistance(response)}
            placement="top-end"
            button={
              <button
                type="button"
                className="flex items-center gap-1 rounded-sm px-2 py-1 text-caption-sm-regular text-white transition-opacity hover:opacity-90"
                style={{
                  background: "linear-gradient(270deg, #6366F1, #8B5CF6, #EC4899, #6366F1)",
                  backgroundSize: "400% 400%",
                  animation: "ai-gradient 6s ease infinite",
                }}
                onClick={() => setGptAssistantModal((prev) => !prev)}
              >
